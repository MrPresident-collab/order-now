...
export function Lojas() {
  ...
  const load = () => {
    setLoading(true);
    setError(null);
    listActiveBusinesses('lojas').then(setBusinesses).catch((e: Error) => setError(e.message)).finally(() => setLoading(false));
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 pb-24">
      <div className="bg-white dark:bg-gray-900 px-5 pt-12 pb-3 safe-top sticky top-0 z-30 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-extrabold text-gray-900 dark:text-white">Lojas</h1>
          </div>
          <ThemeToggle />
        </div>
      ...
      <div className="px-5 pt-4 space-y-3">
        {loading && <LoadingView message="A carregar lojas..." />}
        ...
        {!loading && !error && filtered.map((b) => (
          ...
          >
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">{b.description || 'Loja parceira Pedejá'}</p>
          </button>
        ))}
